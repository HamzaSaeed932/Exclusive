...
const ProductCard = ({item}) => {
    ...
    <div className='w-[270px] h-[350px]  font-poppins relative mr-[30px]'>
        ...
        <div className='w-[270px] h-[250px]  flex justify-center items-center bg-gray-100 rounded relative parent-div'>
            <button className='absolute top-3 left-3 px-3 py-1 bg-[#DB4444] rounded text-xs  text-[#FAFAFA]'>{item.percentage}%</button>
        ...
        <div className='pt-4 flex flex-col gap-2'>
            ...
            <p className='text-[#DB4444]'>${item.newPrice} <span className='text-[rgba(0,0,0,0.5)] line-through pl-3'>${item.prevPrice}</span></p>
            
            <Rating rating={item.rating} value={item.ratingValue}/>
        </div>
